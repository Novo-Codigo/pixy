...
const BadgeModal = ({ badgeId, onClose }: { badgeId: number; onClose: () => void }) => {
            ...
            <div className="relative w-full max-w-md scale-100 transform rounded-2xl bg-white p-6 shadow-2xl transition-all">
                ...
                <div className="flex flex-col items-center gap-4 text-center">
                    <div className="relative">
                        <div
                            className={`rounded-full p-1 ${badge.unlocked ? 'bg-linear-to-tr from-yellow-400 to-orange-500' : 'bg-gray-300'}`}
                        >
                            <img
                                src={badge.icon}
                                alt={badge.title}
                        ...
                        {!badge.unlocked && (
                            <div className="absolute bottom-0 right-0 bg-gray-700 text-white p-2 rounded-full">
                                <FaLock size={12} />
                            </div>
                        )}
                    </div>
                    ...
                    <div>
                        ...
                        <span
                            className={`px-3 py-0.5 rounded-full text-xs font-medium ${badge.unlocked ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}
                        >
...
export default function ProfessionalProgress() {
    ...
    const getRankStyle = (pos: number) => {
        switch (pos) {
            ...
            case 3:
                return 'text-amber-700 border-amber-700 bg-orange-50';
            default:
    ...
    return (
        <div className="min-h-screen w-full bg-linear-to-br from-primary via-secondary to-primary p-6 md:p-12 font-sans text-slate-800 selection:bg-purple-300">
                    ...
                    <section className="flex flex-col gap-3">
                        ...
                        <div className="flex h-80 flex-col overflow-hidden rounded-2xl border border-white/10 bg-white/10 backdrop-blur-md shadow-xl">
                            <div className="flex justify-between border-b border-white/10 p-4 text-white">
                                ...
                                <span className="text-xs text-white/60 self-center uppercase tracking-wider">
                                    Top 100
                                </span>
                            </div>
                                ...
                                {mockedRankingUsers.map((user) => (
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            <div
                                                className={`flex h-8 w-8 items-center justify-center rounded-full border-2 text-xs font-bold ${user.name === 'You' ? 'border-white text-white' : getRankStyle(user.pos)}`}
                                            >
                                                <FaUser />
                                            </div>
                        ...
                        <div className="rounded-2xl border border-white/10 bg-white/10 backdrop-blur-md p-6 shadow-xl">
                            <div className="grid grid-cols-4 gap-4 place-items-center">
                                {mockedBagdesData.map((badge) => (
                                    ...
                                    >
                                        <div
                                            className={`absolute inset-0 rounded-full opacity-20 transition-opacity group-hover:opacity-40 ${badge.unlocked ? 'bg-yellow-400 shadow-[0_0_15px_rgba(250,204,21,0.5)]' : 'bg-gray-500'}`}
                                        ></div>
                                        <img
                                            src={badge.icon}
                                            alt={badge.title}
                    ...
                    <div className="flex flex-1 flex-col justify-center rounded-2xl border border-white/10 bg-white/90 shadow-2xl py-8 relative overflow-hidden">
                        ...
                        <div className="text-center mb-8 px-6">
                            ...
                            <p className="text-gray-500 text-sm mt-1">
                                Deslize para ver os próximos marcos
                            </p>
                        </div>
                        ...
                        <div className="flex w-full overflow-x-auto snap-x snap-mandatory scrollbar-hide pb-8 px-4">
                            {mockedJourneyPoints.map((path, index) => {
                                ...
                                const prevCompleted =
                                    index > 0 && mockedJourneyPoints[index - 1].status !== 'locked';
                                const lineLeftColor =
                                    prevCompleted || isCompleted || isCurrent
                                        ? 'bg-purple-500'
                                        : 'bg-gray-200';

                                const lineRightColor = isCompleted
                                    ? 'bg-purple-500'
                                    : 'bg-gray-200';
                                ...
                                return (
                                    <div
                                        key={path.id}
                                        className="min-w-[50%] relative flex flex-col items-center snap-center group"
                                    >
                                        <div className="absolute top-8 w-full flex items-center justify-center z-0">
                                            <div
                                                className={`h-1.5 w-1/2 ${index === 0 ? 'opacity-0' : ''} ${lineLeftColor} transition-colors duration-500`}
                                            ></div>

                                            <div
                                                className={`h-1.5 w-1/2 ${index === mockedJourneyPoints.length - 1 ? 'opacity-0' : ''} ${lineRightColor} transition-colors duration-500`}
                                            ></div>
                                        </div>
                                        ...
                                        <div className="text-center mt-6 px-2 transition-opacity duration-300">
                                            <p
                                                className={`font-bold text-base truncate w-full ${isCurrent ? 'text-purple-700' : isLocked ? 'text-gray-400' : 'text-gray-700'}`}
                                            >
                                                {path.title}
                                            </p>
            ...
            {badgeSelected && (
                <BadgeModal badgeId={badgeSelected} onClose={() => setBadgeSelected(null)} />
            )}
        </div>
    );
}
